...
function posterity_has_active_sidebar() {
	...
	$wc_active = posterity_is_woocommerce_activated();
	$shop_with_sidebar = $wc_active && posterity_is_woocommerce_sidebar_page();


	if ( $shop_with_sidebar ) {
		$test = 'shop-widget-area';
	} elseif ( $wc_active && posterity_is_woocommerce() ) {
		return false;
		...
		$meta_id        = get_the_ID();
		$custom_sidebar = $posterity_a13->posterity_get_meta( '_sidebar_to_show', $meta_id );
...
function posterity_what_page_type_is_it() {
	...
	if ( empty( $types ) ) {
		$types = array(
			...
			'shop'        => posterity_is_woocommerce_activated() && posterity_is_woocommerce(),
			'product'     => posterity_is_woocommerce_activated() && posterity_is_woocommerce() && is_product(),
		);
		...
		$types['page'] = $types['page'] || ($types['404'] && ($posterity_a13->get_option( 'page_404_template_type' ) === 'custom')) || defined('POSTERITY_CUSTOM_PASSWORD_PROTECTED');
	}
...
function posterity_body_classes( $classes ) {
	...
	if ( is_active_sidebar( 'side-widget-area' ) ) {
		$side   = $posterity_a13->get_option( 'hidden_sidebar_side' );
		$effect = (int) $posterity_a13->get_option( 'hidden_sidebar_effect' );
		if ( $side === 'right' ) {
	...
	//widgets top margin
	if($posterity_a13->get_option( 'widgets_top_margin' ) === 'off'){
	...
	if($header_type === 'vertical'){
		if(is_rtl()){
			$classes[] = 'header-side-'.$posterity_a13->get_option( 'header_side_rtl' );
		}
		else{
			$classes[] = 'header-side-'.$posterity_a13->get_option( 'header_side' );
		}
	}

	//site layout
	$layout_type = $posterity_a13->get_option( 'layout_type' );
	$classes[] = 'site-layout-'.$layout_type;
	if($layout_type === 'bordered'){
		$borders = array( 'top', 'left', 'bottom', 'right' );
		$borders_on = $posterity_a13->get_option( 'theme_borders' );
	...
	//sticky one page
	if( $posterity_a13->posterity_get_meta( '_content_sticky_one_page' ) === 'on'){
		$classes[] = 'a13-body-sticky-one-page';
	}

	//page with posts list
	if ( $page_type['blog_type'] && ! defined( 'POSTERITY_NO_RESULTS' ) ) {
		$classes[] = 'posts-list';
	}

	//cart and others not sidebar/title pages of woocommerce
	if ( $page_type['shop'] && ! posterity_is_woocommerce_sidebar_page() ) {
	...
	//password protected
	if ( defined( 'POSTERITY_PASSWORD_PROTECTED' ) ) {
		$classes[] = 'password-protected';
	}

	//custom password page
	if( defined( 'POSTERITY_CUSTOM_PASSWORD_PROTECTED' ) ){
	...
	if( $page_type['404'] ){
		//custom 404 page
		if( $posterity_a13->get_option( 'page_404_template_type' ) === 'custom' ){
			$classes[] = 'custom404';
			$classes[] = 'page';
		}
		else{
...
function posterity_get_mid_classes() {
	...
	elseif($page_type['shop'] && !posterity_is_woocommerce_sidebar_page()){
		$layout = $posterity_a13->get_option( 'shop_no_major_pages_content_layout' );
	}
	//wish list
	...
	elseif ( $page_type['shop'] && ! $page_type['product'] ) {
		$layout = $posterity_a13->get_option( 'shop_content_layout' );

		//only on pages where list of products are displayed
		if ( is_shop() || is_product_taxonomy() ) {
			$mid_classes[] = 'shop-columns-'.$posterity_a13->get_option( 'shop_products_columns' );
		}
	} //product
	elseif ( $page_type['product'] ) {
		$layout = $posterity_a13->get_option( 'product_content_layout' );
	} //page
	elseif ($page ) {
		$layout_option = $posterity_a13->posterity_get_meta( '_content_layout', $meta_id );
		$layout        = $layout_option === 'global' ? $posterity_a13->get_option( 'page_content_layout' ):
			$layout_option;

		//in content padding
		$top_bottom_padding = $posterity_a13->posterity_get_meta('_content_padding');
		...
		$side_padding = $posterity_a13->posterity_get_meta('_content_side_padding');
		if($side_padding === 'off'){
			$mid_classes[] = 'no-side-space';
		}
	} //single post
	elseif ( $post ) {
		$layout = $posterity_a13->get_option( 'post_content_layout' );
	} //blog type
	elseif ( $page_type['blog_type'] ) {
		$layout = $posterity_a13->get_option( 'blog_content_layout' );

		//in content padding
		$top_bottom_padding = $posterity_a13->get_option( 'blog_content_padding' );
		if($top_bottom_padding === 'top'){
			$mid_classes[] = 'no-bottom-space';
		}
		elseif($top_bottom_padding === 'bottom'){
	...
	if ( 
		 $attachment ||
	     posterity_has_active_sidebar() === false
	) {
		$force_full_width = true;
	}

	function posterity__inner__set_full_width( &$mid_classes ) {
		define( 'POSTERITY_NO_SIDEBARS', true ); /* so we don't have to check again in sidebar.php */
		$mid_classes[] = 'no-sidebars';
	}

	function posterity__inner__set_sidebar_class( &$mid_classes, $sidebar ) {
		if ( ( $sidebar == 'off' ) ) {
			posterity__inner__set_full_width( $mid_classes );
		} else {
	...
	function posterity__inner__check_for_rtl_sidebar_version( $option_name ) {
		global $posterity_a13;
		$sidebar = $posterity_a13->get_option( $option_name );
		if ( is_rtl() ){
			$rtl_opposites = array(
			...
			$sidebar = $posterity_a13->get_option( $option_name.'_rtl', $rtl_opposites[$sidebar] );
		}
	...
	elseif ( $shop && ! $product ) {
		posterity__inner__set_sidebar_class( $mid_classes, posterity__inner__check_for_rtl_sidebar_version( 'shop_sidebar' ) );
	} //product type
	elseif ( $product ) {
		posterity__inner__set_sidebar_class( $mid_classes, posterity__inner__check_for_rtl_sidebar_version( 'product_sidebar' ) );
	} //blog type
	elseif ( $page_type['blog_type'] ) {
		posterity__inner__set_sidebar_class( $mid_classes, posterity__inner__check_for_rtl_sidebar_version( 'blog_sidebar' ) );
	} //single post
	elseif ( $post ) {
		posterity__inner__set_sidebar_class( $mid_classes, posterity__inner__check_for_rtl_sidebar_version( 'post_sidebar' ) );
	} //single page
	elseif ( $page ) {
		...
		if( $sidebar === 'G' || $sidebar === '' ){
			$sidebar = posterity__inner__check_for_rtl_sidebar_version( 'page_sidebar' );
		}
		...
		posterity__inner__set_sidebar_class( $mid_classes, $sidebar );
	}
...
function posterity_faster_google_fonts($urls, $relation_type){
	if('preconnect' === $relation_type){
		...
		$standard_fonts = array_keys( $posterity_a13->posterity_get_standard_fonts_list() );
		$options_fonts = array(
			$posterity_a13->get_option( 'nav_menu_fonts' ),
			$posterity_a13->get_option( 'titles_fonts' ),
			$posterity_a13->get_option( 'normal_fonts' ),
			//default to titles fonts as it was in previous versions
			$posterity_a13->get_option( 'logo_fonts', $posterity_a13->get_option( 'titles_fonts' ) ),
		);
