...
if(!function_exists( 'posterity_get_post_meta_data' )){
    /**
     * Returns post meta data like date, author, comments number and categories
     *
     * @since 2.3.0
     */
    function posterity_get_post_meta_data() {
        global $posterity_a13;

        $types      = posterity_what_page_type_is_it();
	    ...
	    //return date
        if(
            ($post && $posterity_a13->get_option( 'post_date') === 'on')
            ||
            ($post_list && $posterity_a13->get_option( 'blog_date') === 'on')
        ){
            $return .= posterity_posted_on();
        }

	    //return updated date
        if(
            ($post && $posterity_a13->get_option( 'post_date') === 'updated')
            ||
            ($post_list && $posterity_a13->get_option( 'blog_date') === 'updated')
        ){
            $return .= esc_html__( 'Updated: ', 'posterity' ).posterity_modified_on();
        }
        ...
        elseif(
		    ($post && $posterity_a13->get_option( 'post_author') === 'on')
		    ||
		    ($post_list && $posterity_a13->get_option( 'blog_author') === 'on')
	    ){
		    $return .= posterity_posted_by_author();
	    }

        //return comments number
        if(
            ($post && $posterity_a13->get_option( 'post_comments') === 'on')
            ||
            ($post_list && $posterity_a13->get_option( 'blog_comments') === 'on')
        ){
            $return .= posterity_post_comments();
        }

        //Categories
        if(
            ($post && $posterity_a13->get_option( 'post_cats') === 'on')
            ||
            ($post_list && $posterity_a13->get_option( 'blog_cats') === 'on')
        ){
...
if(!function_exists( 'posterity_post_meta_data' )){
    /**
     * Outputs post meta data like date, author, comments number and categories
     *
     * @since 2.3.0
     */
    function posterity_post_meta_data() {
        echo posterity_get_post_meta_data(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }
}


if(!function_exists('posterity_posted_on')){
	/**
	 * get HTML for date of post
     * @return string
     */
    function posterity_posted_on() {
        return '<time class="entry-date published updated" datetime="'.esc_attr( get_the_date( 'c' ) ).'" itemprop="datePublished">'.get_the_date().'</time> ';
    }
}


if(!function_exists('posterity_modified_on')){
	/**
	 * get HTML for date of post
     * @return string
     */
    function posterity_modified_on() {
...
if(!function_exists('posterity_posted_by_author')){
	/**
	 * Author of post
	 * @return string
	 */
	function posterity_posted_by_author() {
        /** @noinspection HtmlUnknownAttribute */
        return
            /* translators: %s - author name */
			sprintf(  esc_html__( 'by %s ', 'posterity' ),
                sprintf('<a class="vcard author" href="%1$s" title="%2$s" %4$s><span class="fn" %5$s>%3$s</span></a>',
				    ...
				    get_the_author(),
                    posterity_get_schema_args('author'),
                    posterity_get_schema_args('name')
                )
...
if(!function_exists('posterity_post_comments')){
	/**
	 * comments link
	 *
     * @return string HTML
     */
    function posterity_post_comments() {
...
if(!function_exists('posterity_under_post_content')){
    function posterity_under_post_content() {
        global $posterity_a13;
        ...
        wp_link_pages( array(
                'before' => '<div id="page-links"><span class="page-links-title">'. esc_html__( 'Pages: ', 'posterity' ).'</span>',
                'after'  => '</div>'
	        )
        );
        ...
        //Tags under content
        if(
            ($post && $posterity_a13->get_option( 'post_tags' ) === 'on')
            ||
            ($post_list && $posterity_a13->get_option( 'blog_tags' ) === 'on')
        ){
...
if(!function_exists('posterity_author_info')){
     ...
     */
    function posterity_author_info() {
        global $posterity_a13;

        $author_description =  get_the_author_meta( 'description' );

        if( ( strlen($author_description) > 0 ) && $posterity_a13->get_option( 'post_author_info' ) === 'on'): ?>
            <div class="about-author comment"<?php posterity_schema_args('author_box'); ?>>
                <div class="comment-body">
                    <?php $author_ID = get_the_author_meta( 'ID' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                        echo '<a href="'.esc_url( get_author_posts_url($author_ID) ).'" class="avatar"'.posterity_get_schema_args('url').'>'.get_avatar( $author_ID, 90 ).'</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                        echo '<strong class="comment-author"><span class="author-name" '.posterity_get_schema_args('name').'>'.get_the_author().'</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
if(!function_exists('posterity_posts_navigation')){
     ...
     */
    function posterity_posts_navigation() {
        global $posterity_a13;

        if($posterity_a13->get_option( 'post_navigation' ) === 'on'){
            //posts navigation
            ...
            if($is_prev || $is_next){
                ...
                add_filter( 'posterity_post_navigation_image_size', function(){
                    return array(245,245);
                });
                 *
                 */
                $image_sizes = apply_filters( 'posterity_post_navigation_image_size', array( 245, 100 ) );

                if($is_prev){
                    $id = $prev_post->ID;
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item prev">'
                         .'<span><i class="fa fa-long-arrow-'.( is_rtl() ? 'right' : 'left' ).'"></i> '.esc_html__( 'Previous article', 'posterity' ).'</span>'
                         .'<span class="title">'.esc_html( $prev_post->post_title ).'</span>'
                         .'<span class="image">'.posterity_make_post_image( $id, $image_sizes ).'</span>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                         .'</a>';
                }
                if($is_next){
                    $id = $next_post->ID;
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item next">'
                         .'<span>'.esc_html__( 'Next article', 'posterity' ).' <i class="fa fa-long-arrow-'.( is_rtl() ? 'left' : 'right' ).'"></i></span>'
...
if(!function_exists('posterity_excerpt_length')){
    /**
     ...
     */
    function posterity_excerpt_length($length) {
        ...
        global $posterity_a13;
        return intval( $posterity_a13->get_option( 'blog_excerpt_length') );
    }
}
add_filter( 'excerpt_length', 'posterity_excerpt_length' );


if(!function_exists('posterity_new_excerpt_more')){
    /**
     ...
     * @return string HTML
     */
    function posterity_new_excerpt_more($link) {
        $ajax_for_vc_grid = defined( 'DOING_AJAX' ) && DOING_AJAX && isset($_POST['action']) && ($_POST['action'] === 'vc_get_vc_grid_data');
        ...
        global $post, $posterity_a13;
        if( $posterity_a13->get_option( 'blog_read_more', 'on' ) === 'off' ){
            return '';
        }

        //return read more
        return '&hellip;
 <p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'posterity' ).'</a></p>';
    }
}
add_filter( 'excerpt_more', 'posterity_new_excerpt_more' );



if(!function_exists('posterity_has_excerpt_read_more')){
	/**
	 * Adds read more when excerpt is provided by user
	 *
     * @return string HTML
     */
    function posterity_has_excerpt_read_more($content) {
        global $post, $posterity_a13;

        if( $posterity_a13->get_option( 'blog_read_more', 'on' ) === 'on' && has_excerpt() ){
            $content .= '<p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'posterity' ).'</a></p>';
        }

        return $content;
    }
}
add_filter( 'the_excerpt', 'posterity_has_excerpt_read_more' );


if(!function_exists('posterity_read_more_new_line')){
	/**
	 ...
	 * @return string HTML
	 */
    function posterity_read_more_new_line($link) {
        return '<p>'.$link.'</p>';
    }
}
add_filter( 'the_content_more_link', 'posterity_read_more_new_line' );



if(!function_exists('posterity_comments_navigation')){
	/**
     * Comments navigation
     */
    function posterity_comments_navigation() {
        if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
            ?>
            <nav class="navigation comment-navigation">
                <h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'posterity' ); ?></h2>
                <div class="nav-links">
                    <?php
                    if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'posterity' ) ) ) :
                        printf( '<div class="nav-previous">%s</div>', wp_kses_post( $prev_link ) );
                    endif;

                    if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'posterity' ) ) ) :
...
if(!function_exists('posterity_daoon_chat_post')){
    /**
...
if(!function_exists('posterity_display_items_from_query_post_list')) {
     ...
     * @param array         $args
     */
    function posterity_display_items_from_query_post_list($query = false, $args = array()){
        global $posterity_a13;
        ...
        $default_args = array(
            'columns'         => $posterity_a13->get_option( 'blog_brick_columns' ),
            'max_width'       => $posterity_a13->get_option( 'blog_bricks_max_width' ),
            'margin'          => $posterity_a13->get_option( 'blog_brick_margin' ),
            'filter'          => false,
        ...
        if($args['filter']){
            ...
            posterity_make_post_grid_filter($terms, 'posts-filter', $args['default_filter'], $args['all_filter']);
        }
        ...
        if ( ! $query->have_posts() ):
            ...
            <div class="formatter">
                <div class="real-content empty-blog">
                    <?php
                    if( !is_search() ){
                        echo '<p>'.esc_html__( 'Apologies, but no results were found for the requested archive.', 'posterity' ).'</p>';
                    }
            ...
            $posts_classes = '';
            $posts_layout_class = $posterity_a13->get_option( 'blog_post_look');
            ...
            if($post_list_page){
                ...
                $lazy_load_mode   = $posterity_a13->get_option( 'blog_lazy_load_mode' );
            ...
            while ( $query->have_posts() ) : $query->the_post();
                ...
                //size of brick
                $brick_size = ($posts_layout_class === 'horizontal') ? 1 : $posterity_a13->posterity_get_meta( '_brick_ratio_x' );
                ...
                posterity_schema_args('creative');
                echo '>';

                    if(post_password_required()){
                        posterity_top_image_video($link_it, array_merge($args, array('page_type' => 'blog_type', 'post_grid' => true ) ) );
                        ?>
                        <div class="formatter">
                            <?php the_title('<h2 class="post-title entry-title"><a href="'. esc_url( get_permalink() ) . '"><span class="fa fa-lock"></span>', '</a></h2>'); ?>
                            <div class="real-content">
                                <p><?php esc_html_e( 'To view it please enter your password below', 'posterity' ); ?></p>
                                <?php echo posterity_password_form(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped  ?>
                            </div>
                        </div>
                        <?php
                    }
            ...
            echo '</div></div>';
            if(!$ajax_call){
                posterity_post_list_individual_look($id, $args);
            }
        endif;
    }
}
...
if ( ! function_exists( 'posterity_next_posts_link_class' ) ) {
	/**
	 * Adding class for compatibility with Wp-paginate plugin + infinite scroll configuration
	 *
	 * @return string
	 */
	function posterity_next_posts_link_class() {
		return 'class="next"';
	}
}

if ( ! function_exists( 'posterity_prev_posts_link_class' ) ) {
	/**
	 * Adding class for compatibility with Wp-paginate plugin + infinite scroll configuration
	 *
	 * @return string
	 */
	function posterity_prev_posts_link_class() {
		return 'class="prev"';
	}
}
add_filter( 'next_posts_link_attributes', 'posterity_next_posts_link_class' );
add_filter( 'previous_posts_link_attributes', 'posterity_prev_posts_link_class' );



if(!function_exists('posterity_post_list_individual_look')){
    /**
     ...
     */
    function posterity_post_list_individual_look($id, $args){
        $css = '';

        $max_width        = (int) $args['max_width'];
        $max_width        = esc_html( posterity_make_css_rule( 'max-width', $max_width, '%spx' ) );
	...
	.posts-list .layout-full_padding.with-sidebar .content-box{
        ...
        if(strlen($css)){
            ...
            if ( defined('ELEMENTOR_VERSION') && \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
                ...
                echo '<style type="text/css" media="all" id="a13-posts-grid-'.esc_attr($id).'">'.$css.'</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            }
            // not edit mode
            else {
...
if(!function_exists('posterity_get_schema_args')){
    /**
     ...
     */
    function posterity_get_schema_args( $for ){
        $prop = '';
        $scope = false;
        $type = '';
        ...
        $attributes = apply_filters( 'posterity_schema_args', $attributes, $for );
        /* SAMPLE USAGE */
        /*
        add_filter( 'posterity_schema_args', function($args, $for){
                if($for == 'body'){
                    $args['type'] = 'Article';
                }
...
if(!function_exists('posterity_schema_args')){
    /**
     ...
     * @return      string - string of HTML attributes
     */
    function posterity_schema_args( $for ) {
        echo posterity_get_schema_args( $for ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }
}

if(!function_exists('posterity_remove_wpseo')){
    /**
     ...
     *
     */
    function posterity_remove_wpseo() {
...
add_action( 'template_redirect', 'posterity_remove_wpseo' );