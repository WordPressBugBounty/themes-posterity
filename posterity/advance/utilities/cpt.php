<?php
if(!function_exists( 'posterity_cpt_meta_fields' )){
	/**
	 * Return internet address and custom fields of post
	 *
	 * @return string
	 */
    function posterity_cpt_meta_fields(){
        $fields = '';
	    $custom_fields_number = apply_filters('posterity_custom_fields_number', 5);
        ...
        for($i = 0; $i <= $custom_fields_number; $i++){
	        if( $i === 0 ) {
		        ...
		        if ( strlen( $temp ) ) {
			        $temp = esc_html__( 'Website', 'posterity' ).':'.$temp;
		        }
	        }
	        else{
		        //custom fields
		...
		if(strlen($fields)){
			$html =
				'<div class="meta-data">'
			       ...
			       .apply_filters('posterity_after_cpt_meta_fields','').
		        '</div>';
		}
...
if(!function_exists( 'posterity_cpt_meta_data' )){
	/**
	 ...
	 * @return string
	 */
    function posterity_cpt_meta_data( $content ){
	    return $content . posterity_cpt_meta_fields();
    }
}


if(!function_exists( 'posterity_cpt_social' )){
	/**
	 ...
	 *
	 */
    function posterity_cpt_social( $link, $title ){
...
if(!function_exists('posterity_cpt_list_navigation')){
	 ...
	 */
	function posterity_cpt_list_navigation( $query = false ){
		if($query === false){
			global $wp_query;
			$query = $wp_query;
		}
		...
		$links = paginate_links( apply_filters( 'posterity_pagination_args', array(
			'add_args'     => '',
...
if(!function_exists('posterity_loop_pagination')){
	/**
	 ...
	 * @return array
	 */
	function posterity_loop_pagination($args){
		$args['type'] = 'plain';
		$args['prev_text'] = _x( 'Previous', 'previous product', 'posterity' );
		$args['next_text'] = _x( 'Next', 'next product', 'posterity' );

		return $args;
	}
}
add_filter('posterity_pagination_args', 'posterity_loop_pagination' );


if(!function_exists('posterity_make_media_collection')){
	/**
	 ...
	 */
    function posterity_make_media_collection($id = null, $args = array()){
	    if(is_null($id)){
	        $id = get_the_ID();
	    }
	    ...
	    <ul class="gallery-media-collection screen-reader-text">
			<?php echo posterity_prepare_frontend_gallery_html( posterity_prepare_gallery_attachments( $value, $id ), $order, $id, $args ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		</ul>
	    <?php
    }
}
...
function posterity_prepare_gallery_attachments( $value, $post_id = null ){
	...
	if ( ! empty( $value ) ) {
		...
		$proofing_enabled = $posterity_a13->posterity_get_meta( '_proofing', $post_id ) === 'on';
		...
		if ( $media_count ) {
			...
			posterity_prepare_external_media( $externals );
...
function posterity_prepare_external_media(&$items){
	...
	$proofing_enabled = $posterity_a13->posterity_get_meta( '_proofing' ) === 'on';
		...
		foreach ( $attachments as $item ) {
			...
			elseif( is_array($item) && $item['type'] === 'audio' ){
				...
				if ( $item['id'] === 'external' ) {
					...
					//video details
					$temp =  posterity_detect_movie( $item['videolink_link'] );
					...
					$collector[ 'video_player' ]   = posterity_get_movie_link( $temp );       //Video API address
					...
					list(
						...
						$collector[ 'thumb' ]
						) = posterity_get_gallery_item_images($item, $collector, $columns, $bricks_max_width, $brick_margin, $brick_proportion);
			...
			posterity_frontend_gallery_item_html( $collector, $id, $args );
...
function posterity_frontend_gallery_item_html( $collector, $post_id, $args ){
	...
	if(strlen($collector[ 'filter_tags' ])){
		$album_filters = posterity_get_media_filters($post_id);
		$item_filters = array();
		$data_filter = '';
	...
	?>
	<li class="<?php echo esc_attr($collector[ 'item_class' ]); ?>"<?php echo esc_html($data_attr); ?>>
		...
		if( $collector[ 'attachment_type' ] === 'internal' && $collector[ 'type' ] === 'video' ){
			...
			//posterity_video() produces escaped content
			echo '<div class="album-video" id="album-video-'.esc_attr( $id ).'">'.posterity_video( $video_attr ).'</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		...
		if ( !$proofing_enabled ) {
			if(function_exists( 'ADDTOANY_SHARE_SAVE_KIT' )){
				...
				if ( $posterity_a13->get_option( 'album_social_icons' ) === 'on' && $sharing_enabled ){

					$back_to_post = $posterity_a13->get_option( 'album_share_type' ) === 'album';
					//share link will link to album and open image in lightbox
					if($back_to_post){
						$album_url = posterity_current_url();
...
//				    ADDTOANY_SHARE_SAVE_KIT( array( 'linkname' => $collector['title'], 'linkurl' => $collector['src'] ) );
		...
		//add to cart button
		if( posterity_is_woocommerce_activated() && isset( $collector['product_id'] ) && $collector[ 'product_id' ]){
...
if ( ! function_exists( 'posterity_cpt_as_frontpage_title_fix' ) ) {
	 ...
	 */
	function posterity_cpt_as_frontpage_title_fix( $title ) {
...
if ( ! function_exists( 'posterity_cpt_as_frontpage_menu_fix' ) ) {
	/**
	 ...
	 */
	function posterity_cpt_as_frontpage_menu_fix( $classes, $item ) {
