...
if(!function_exists('posterity_page_preloader')){
	/**
	 * Prints page preloader screen
	 */
	function posterity_page_preloader(){
        global $posterity_a13;

        if($posterity_a13->get_option( 'preloader' ) === 'on'){
	        $preloader_type = $posterity_a13->get_option(  'preloader_type' );
	        $class_attr = $preloader_type;
	        if($posterity_a13->get_option( 'preloader_hide_event' )==='ready'){
		        $class_attr .= ' onReady';
	        }
        ?>
<div id="preloader" class="<?php echo esc_attr($class_attr); ?>">
    <div class="preload-content">
        <div class="preloader-animation"><?php posterity_preloader_animation_html($preloader_type); ?></div>
...
function posterity_page_preloader_css() {
	...
	//prelaoder
	if ( $posterity_a13->get_option( 'preloader' ) === 'on' ) {
		$prelaoder_bg_color     = posterity_make_css_rule( 'background-color', $posterity_a13->get_option_color_rgba( 'preloader_bg_color' ) );
		$prelaoder_bg_image     = posterity_make_css_rule( 'background-image', $posterity_a13->get_option_media_url( 'preloader_bg_image' ), 'url(%s)' );
		$prelaoder_bg_image_fit = posterity_bg_fit_helper( $posterity_a13->get_option( 'preloader_bg_image_fit' ) );
...
#preloader{
    ...
    $prelaoder_bg_image_fit
		...
		$prelaoder_type  = $posterity_a13->get_option( 'preloader_type' );
		if ( $prelaoder_type !== 'none' ) {
			$preloader_css_file = get_theme_file_path( 'css/preloaders/' . $prelaoder_type . '.css' );
			$content = posterity_read_contents( $preloader_css_file );

			if( $content !== false ){
				$prelaoder_color = $posterity_a13->get_option_color_rgba( 'preloader_color' );
...
if(!function_exists('posterity_preloader_animation_html')){
	/**
	 * Prints one of few animations for preloader screen
	 *
	 * @param string $animation name of animation
	 */
	function posterity_preloader_animation_html($animation) {
		switch($animation){
...
if(!function_exists('posterity_page_background')){
	/**
	 * Prints page background element
	 */
	function posterity_page_background(){
        ?>
...
function posterity_page_background_css() {
	...
	$global_bg_color   = posterity_make_css_rule( 'background-color', $posterity_a13->get_option_color_rgba( 'appearance_body_bg_color' ) );
	$global_image      = posterity_make_css_rule( 'background-image', $posterity_a13->get_option_media_url( 'appearance_body_image' ), 'url(%s)' );
	$global_image_fit  = posterity_bg_fit_helper( $posterity_a13->get_option( 'appearance_body_image_fit' ) );
	$error404_bg_image = posterity_make_css_rule( 'background-image', $posterity_a13->get_option_media_url( 'page_404_bg_image' ), 'url(%s)' );
	$password_bg_image = posterity_make_css_rule( 'background-image', $posterity_a13->get_option_media_url( 'page_password_bg_image' ), 'url(%s)' );
...
//single pages
if ( $posterity_a13->get_option( 'page_custom_background' ) === 'on' ) {
	$page_bg_color  = posterity_make_css_rule( 'background-color', $posterity_a13->get_option_color_rgba( 'page_body_bg_color' ) );
	$page_image     = posterity_make_css_rule( 'background-image', $posterity_a13->get_option_media_url( 'page_body_image' ), 'url(%s)' );
	$page_image_fit = posterity_bg_fit_helper( $posterity_a13->get_option( 'page_body_image_fit' ) );
...
//blog
if ( $posterity_a13->get_option( 'blog_custom_background' ) === 'on' ) {
	$blog_bg_color  = posterity_make_css_rule( 'background-color', $posterity_a13->get_option_color_rgba( 'blog_body_bg_color' ) );
	$blog_image     = posterity_make_css_rule( 'background-image', $posterity_a13->get_option_media_url( 'blog_body_image' ), 'url(%s)' );
	$blog_image_fit = posterity_bg_fit_helper( $posterity_a13->get_option( 'blog_body_image_fit' ) );
...
//shop
if ( $posterity_a13->get_option( 'shop_custom_background' ) === 'on' ) {
	$shop_bg_color  = posterity_make_css_rule( 'background-color', $posterity_a13->get_option_color_rgba( 'shop_body_bg_color' ) );
	$shop_image     = posterity_make_css_rule( 'background-image', $posterity_a13->get_option_media_url( 'shop_body_image' ), 'url(%s)' );
	$shop_image_fit = posterity_bg_fit_helper( $posterity_a13->get_option( 'shop_body_image_fit' ) );
...
if(!function_exists('posterity_theme_borders')){
	/**
	 * Prints theme border if enabled
	 */
	function posterity_theme_borders(){
		global $posterity_a13;

		if($posterity_a13->get_option( 'layout_type' ) === 'bordered') {
			?>
...
if(!function_exists('posterity_search_form')){
	/**
	 ...
	 * @return string HTML
	 */
	function posterity_search_form(
	    ...
	    $shop_search_option = $posterity_a13->get_option( 'shop_search' );
	    $shop_search = $shop_search_option === 'on';
        $helper_search = get_search_query() == '' ? true : false;
        $field_search = '<input' .
            ' placeholder="' . esc_attr__('Search &hellip;', 'posterity' ) . '" ' .
        ...
        $form = '
                <form class="search-form" method="get" action="' . esc_url( home_url( '/' ) ) . '" >
                    <fieldset class="semantic">
                        ' . $field_search . '
                        <input type="submit" id="searchsubmit' . esc_attr( $search_id ) . '" title="'. esc_attr_x( 'Search', 'action', 'posterity' ) .'" value="'. esc_attr_x( 'Search', 'action', 'posterity' ) .'" />
...
add_filter( 'get_search_form','posterity_search_form' );



if(!function_exists('posterity_get_title_bar')){
	/**
	 ...
	 * @return string|void
	 */
	function posterity_get_title_bar( $called_position = 'outside', $title = '', $subtitle = '') {
        global $posterity_a13;
        ...
        if($page_type['product']){
	        $title_bar_option = $posterity_a13->get_option( 'product_title', 'on' );
	        $position         = $posterity_a13->get_option( 'product_title_bar_position', 'inside' );
	        $title_bar_variant = $posterity_a13->get_option( 'product_title_bar_variant' );
	        $title_bar_width = $posterity_a13->get_option( 'product_title_bar_width' );

	        //parallax
	        $parallax = $posterity_a13->get_option( 'product_title_bar_parallax' ) === 'on';

	        if ( $parallax ) {
		        $parallax_type = $posterity_a13->get_option( 'product_title_bar_parallax_type' );
		        $parallax_speed = $posterity_a13->get_option( 'shop_title_bar_parallax_speed' );
        ...
        elseif( ( $page_type['shop'] && !posterity_is_woocommerce_sidebar_page() ) ||
	        ...
	        $title_bar_option = $posterity_a13->get_option( 'shop_no_major_pages_title' );
	        //position is not overwritten
	        $title_bar_variant = $posterity_a13->get_option( 'shop_no_major_pages_title_bar_variant' );
	        $title_bar_width = $posterity_a13->get_option( 'shop_no_major_pages_title_bar_width' );

	        //parallax
	        $parallax = $posterity_a13->get_option( 'shop_no_major_pages_title_bar_parallax' ) === 'on';

	        if ( $parallax ) {
		        $parallax_type = $posterity_a13->get_option( 'shop_no_major_pages_title_bar_parallax_type' );
		        $parallax_speed = $posterity_a13->get_option( 'shop_title_bar_parallax_speed' );
		        $tb_classes .= ' a13-parallax';
		        $data_attr .= ' data-a13-parallax-type="'.esc_attr($parallax_type).'" data-a13-parallax-speed="'.esc_attr((float)$parallax_speed).'"';
	        }
        ...
        elseif($page_type['shop']){
	        $title_bar_option = $posterity_a13->get_option( 'shop_title' );
	        //position is not overwritten
	        $title_bar_variant = $posterity_a13->get_option( 'shop_title_bar_variant' );
	        $title_bar_width = $posterity_a13->get_option( 'shop_title_bar_width' );

	        //parallax
	        $parallax = $posterity_a13->get_option( 'shop_title_bar_parallax' ) === 'on';

	        if ( $parallax ) {
		        $parallax_type = $posterity_a13->get_option( 'shop_title_bar_parallax_type' );
		        $parallax_speed = $posterity_a13->get_option( 'shop_title_bar_parallax_speed' );
		        $tb_classes .= ' a13-parallax';
		        $data_attr .= ' data-a13-parallax-type="'.esc_attr($parallax_type).'" data-a13-parallax-speed="'.esc_attr((float)$parallax_speed).'"';
	        }
        ...
        elseif($page_type['blog_type']){
	        $title_bar_option = $posterity_a13->get_option( 'blog_title' );
	        //position is not overwritten
	        $title_bar_variant = $posterity_a13->get_option( 'blog_title_bar_variant' );
	        $title_bar_width = $posterity_a13->get_option( 'blog_title_bar_width' );

	        //parallax
	        $parallax = $posterity_a13->get_option( 'blog_title_bar_parallax' ) === 'on';

	        if ( $parallax ) {
		        $parallax_type = $posterity_a13->get_option( 'blog_title_bar_parallax_type' );
		        $parallax_speed = $posterity_a13->get_option( 'blog_title_bar_parallax_speed' );
		        $tb_classes .= ' a13-parallax';
		        $data_attr .= ' data-a13-parallax-type="'.esc_attr($parallax_type).'" data-a13-parallax-speed="'.esc_attr((float)$parallax_speed).'"';
	        }
        }
        ...
        elseif($page_type['page'] || $page_type['post']){
            $meta_id = get_the_ID();
            $_subtitle = $posterity_a13->posterity_get_meta('_subtitle', $meta_id);
            ...
            if( $title_bar_option === 'global' ){
	            ...
	            $title_bar_option    = $posterity_a13->get_option( $post_type . '_title' );
	            $position            = $posterity_a13->get_option( $post_type . '_title_bar_position' );
	            $title_bar_variant   = $posterity_a13->get_option( $post_type . '_title_bar_variant' );
	            $title_bar_width     = $posterity_a13->get_option( $post_type . '_title_bar_width' );

	            //parallax
	            $parallax = $posterity_a13->get_option( $post_type.'_title_bar_parallax' ) === 'on';

	            if ( $parallax ) {
		            $parallax_type = $posterity_a13->get_option( $post_type.'_title_bar_parallax_type' );
		            $parallax_speed = $posterity_a13->get_option( $post_type.'_title_bar_parallax_speed' );
		            $tb_classes .= ' a13-parallax';
		            $data_attr .= ' data-a13-parallax-type="'.esc_attr($parallax_type).'" data-a13-parallax-speed="'.esc_attr((float)$parallax_speed).'"';
	            }
            }
	            ...
	            $title_bar_variant = $posterity_a13->posterity_get_meta('_title_bar_variant', $meta_id);
	            $title_bar_width = $posterity_a13->posterity_get_meta('_title_bar_width', $meta_id);
	            $display_breadcrumbs = $posterity_a13->posterity_get_meta('_breadcrumbs', $meta_id) === 'on';

	            //parallax
		        $parallax = $posterity_a13->posterity_get_meta('_title_bar_parallax', $meta_id) === 'on';

		        if ( $parallax ) {
			        $parallax_type = $posterity_a13->posterity_get_meta('_title_bar_parallax_type', $meta_id);
			        $parallax_speed = $posterity_a13->posterity_get_meta('_title_bar_parallax_speed', $meta_id);
			        $tb_classes .= ' a13-parallax';
			        $data_attr .= ' data-a13-parallax-type="'.esc_attr($parallax_type).'" data-a13-parallax-speed="'.esc_attr((float)$parallax_speed).'"';
		        }
            }
        ...
        //is it OFF?
        if(!posterity_is_no_property_page() && !$is_password_protected){ //checks if page can have meta fields
		...
		if(strlen($title) === 0){
			...
			if(posterity_is_woocommerce_activated() && posterity_is_woocommerce_products_list_page()){
			...
			elseif ( $home ){
				if(get_option('page_for_posts') === '0'){
					$title =  esc_html__( 'Blog', 'posterity' );
				}
				else{
		...
		if($position === 'inside'){
			$html .= posterity_get_top_image_video(false, array('full_size' => true)); //so small images will look good instead of being stretched
		}
		...
		//main title
		$html .= '<h1 class="page-title entry-title"'.posterity_get_schema_args('headline').'>'.$title.'</h1>';//sometimes we add html here, so don't escape!
...
if(!function_exists('posterity_result_count')){
	 ...
	 */
	function posterity_result_count( $query = false ){
		if($query === false){
			global $wp_query;
			$query = $wp_query;
		}
...
if(class_exists('SearchWP_Live_Search')){
	...
	if(!function_exists('posterity_searchwp_live_search_posts_per_page')) {
		/**
		 *  Control how many results are returned
		 */
		function posterity_searchwp_live_search_posts_per_page() {
			return 6;
		}
	}
	add_filter( 'searchwp_live_search_posts_per_page', 'posterity_searchwp_live_search_posts_per_page' );



	if(!function_exists('posterity_searchwp_live_search_configs')) {
		/**
		 ...
		 * @return
		 */
		function posterity_searchwp_live_search_configs( $configs ) {
	...
	function posterity_make_post_grid_filter( $terms, $filter_class = '', $selected_term = '__all', $show_all = true ) {
		if( count( $terms ) ){
			...
			if( $selected_term === '__all' || $show_all ){
				echo '<li '.( $selected_term === '__all' ? ' class="selected"' : '' ).' data-filter="__all"><a href="' . esc_url( posterity_current_url() ) . '">' . esc_html__( 'All', 'posterity' ) . '</a></li>';
			}
