...
class Posterity_Framework
{
     ...
     * kind of constructor
     */
    function posterity_start()
    {
        /**
         * Define bunch of helpful paths and settings
         */
        define('POSTERITY_TPL_SLUG', 'posterity');//it is not always same as directory of theme
        define('POSTERITY_OPTIONS_NAME_PART', 'Posterity');
        define('POSTERITY_THEME_ID_NUMBER', '66');
        define('POSTERITY_OPTIONS_NAME', 'skt13_option_posterity');
        define('POSTERITY_CACHE', 'skt13_posterity_cache');
        define('POSTERITY_THEME_VERSION', '1.0');
        define('POSTERITY_THEME_VER', POSTERITY_THEME_VERSION ); //legacy - do not use
        define('POSTERITY_MIN_COMPANION_VERSION', '1.0');
        define('POSTERITY_MIN_PHP_VERSION', '5.6');
        define('POSTERITY_MIN_WP_VERSION', '5.0');

        //theme root
        define('POSTERITY_TPL_URI', get_template_directory_uri());

        //plugins recommended by theme
        define('POSTERITY_TPL_PLUGINS', POSTERITY_TPL_URI . '/advance/plugins');
        define('POSTERITY_TPL_PLUGINS_DIR', get_template_directory() . '/advance/plugins');

        //misc theme globals
        define('POSTERITY_INPUT_PREFIX', 'a13_');
        define('POSTERITY_CONTENT_WIDTH', 800);

        //check minimal requirements for WordPress and PHP
        if (
            version_compare( $GLOBALS['wp_version'], POSTERITY_MIN_WP_VERSION, '<' )
            ||
            version_compare( PHP_VERSION, POSTERITY_MIN_PHP_VERSION, '<' )
        ) {
        ...
        if( is_customize_preview() ){
            ...
            add_action( 'wp_loaded', array( $this, 'posterity_customizer_wp_loaded' ) );
            //before save
            add_action('customize_save', array($this, 'posterity_customizer_customize_save_before'));
            //perform option save while using customizer
            add_action('customize_save_after', array($this, 'posterity_customizer_customize_save_after'));
        }
        ...
        if ( is_admin() ) {
            ...
            /** @noinspection PhpIncludeInspection */
			get_template_part('advance/admin/posterityinfo', 'pages');
            /** @noinspection PhpIncludeInspection */
            ...
            if (current_user_can('update_core')) {
                add_action( 'admin_notices', array(&$this, 'posterity_check_for_warnings') );
            }
        }
        ...
        $file_to_test = get_template_part('advance/envy');
        if (is_string($file_to_test) && is_file($file_to_test)) {
            /** @noinspection PhpIncludeInspection */
            require_once $file_to_test; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
        }
		
		
		$file_to_test = get_template_part('advance/posterity');  // Use the appropriate file extension
		if (is_string($file_to_test) && is_file($file_to_test)) {
			/** @noinspection PhpIncludeInspection */
			require_once $file_to_test; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound  
		}
        ...
        add_action('after_setup_theme', array(&$this, 'posterity_setup'));
    }
     ...
     */
    function posterity_set_sections($section){
        /**
         * @since 2.3.0
         */
        do_action( 'posterity_options_before_'.$section['id'] );
         ...
         * @since 2.3.0
         */
        do_action( 'posterity_options_after_'.$section['id'] );
    }
     ...
     */
    function posterity_get_sections(){
     ...
     * @return array
     */
    function posterity_set_settings_set($set, $values){
     ...
     * @since 2.3.0
     */
    function posterity_get_settings_set($set){
     ...
     */
    function posterity_customizer_wp_loaded() {
        $this->theme_options = get_option(POSTERITY_OPTIONS_NAME);
        $this->load_options();
    }
     ...
     */
    function posterity_customizer_customize_save_before()
    {
     ...
     */
    function posterity_customizer_customize_save_after()
    {
        //get new set of options
        $this->theme_options = get_option(POSTERITY_OPTIONS_NAME);
        $this->load_options();

        do_action( 'posterity_generate_user_css' );

        //refresh cache
        delete_option(POSTERITY_CACHE);
    }

    /**
     * Various setup actions for setting up theme for WordPress
     */
    function posterity_setup()
    {
        global $content_width;
        //content width
        if (!isset($content_width)) {
            $content_width = POSTERITY_CONTENT_WIDTH;
        }
        ...
        if (
            ...
            ( function_exists('posterity-a13fe_user_css_name') && ! file_exists( posterity-a13fe_user_css_name() ) ) ||
        ...
        //LANGUAGE
        load_theme_textdomain( 'posterity', get_theme_file_path( 'languages' ) );
        ...
        register_nav_menus(array(
            'header-menu' => __( 'Site Navigation', 'posterity' ),
        ));
    }
     ...
     */
    function posterity_check_for_warnings()
    {
        $notices = array();
        $valid_tags = array(
            'a' => array(
        ...
        foreach ($notices as $id => $notice) {
            ...
            if( !posterity_is_admin_notice_active($id) ){
                continue;
            }
        ...
        do_action( 'posterity_theme_notices' );
    }
    ...
    public function set_options( $overload_options = array() )
    {
        if( is_array($overload_options) && count($overload_options) > 0){
            update_option(POSTERITY_OPTIONS_NAME, $overload_options);
    ...
    public function get_option($index, $default = '', $filter = true)
    {
        ...
        return apply_filters('posterity_a13_options_'.$index, $option_to_return );
    }
    ...
    public function get_option_media_url($index)
    {
        ...
        elseif( is_string($option) && ( strlen($option) > 0 ) ){
            if(strncmp($option, "http", 4) !== 0){
                ...
                $option = POSTERITY_TPL_URI . $option;
            }
        }
    ...
    public function get_option_color_rgba( $index )
    {
        ...
        if ( is_array( $option ) ) {
            if ( isset( $option['rgba'] ) ) {
                ...
                return posterity_hex2rgba( $option['color'], $option['alpha'] ); //we got RGBA
            } else {
                return ''; //empty string as it is probably not set yet
            }
        }
...
//            'images_manager' //no parent options here
        ...
        foreach ($option_func as $function) {
            $function_to_call = 'posterity_meta_boxes_' . $function;
    ...
    public function get_meta_required_array() {
        ...
        if(strlen($post_type)){
            switch ( $post_type ) {
                case 'post':
                    $meta_boxes = posterity_meta_boxes_post();
                    break;
                case 'page':
                    $meta_boxes = posterity_meta_boxes_page();
                    break;
            }
    ...
    public function prepare_theme_vars(){
        $cache = get_option( POSTERITY_CACHE );
        ...
        elseif( $cache === false || $cache['version'] !== POSTERITY_THEME_VERSION || defined( 'ICL_SITEPRESS_VERSION') || defined( 'POLYLANG_BASENAME' ) ){
            ...
            //normal flow
                ...
                $cache = array(
                    ...
                    'version' => POSTERITY_THEME_VERSION
                );

                //save cache
                update_option( POSTERITY_CACHE, $cache );
            }
        }
        //cache hit
        else{
     ...
     */
    function posterity_get_meta($field, $id = false)
    {
        $family = '';

        if (!$id && posterity_is_no_property_page()) {
     ...
     */
    function posterity_get_social_icons_list($what = 'names'){
        $icons = array(
    ...
    function posterity_get_standard_fonts_list(){
        return array(
    ...
    function posterity_is_companion_plugin_ready($fail_message = false, $silent = false){
        ...
        if ( empty( $plugins[$plugin_file] ) ) {
            ...
            $label = __( 'Install', 'posterity' ) . ' : ' . __( 'SKT Templates – Elementor & Gutenberg templates', 'posterity' );
        }
        ...
        elseif ( is_plugin_inactive( $plugin_path ) ){
            ...
            $label = __( 'Activate', 'posterity' ) . ' : ' . __( 'SKT Templates – Elementor & Gutenberg templates', 'posterity' );
        }

        //not up to date
        elseif( version_compare( $plugins[$plugin_file]['Version'], POSTERITY_MIN_COMPANION_VERSION, '<'  ) ){
            $ready = false;
            wp_enqueue_script( 'updates' );
            $classes =  ' update-now';
            ...
            $label = __( 'Update', 'posterity' ) . ' : ' . __( 'SKT Templates – Elementor & Gutenberg templates', 'posterity' );
        }

        if(!$ready && !$silent){
            $message = $fail_message ? $fail_message : __( 'This feature requires SKT Templates plugin to be active and in the proper version.', 'posterity' );
