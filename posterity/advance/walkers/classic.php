...
class POSTERITY_menu_walker extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if( $is_nava ){
            ...
            if( $nava_page_slug != $current_page_slug ){
                ...
                //it is on sub-page
                    ...
                    $url = get_home_url(null, $nava_page_slug);
                }
        ...
        if( $posterity_a13->get_option( 'header_type' ) === 'vertical' &&
            $posterity_a13->get_option( 'submenu_active_open', 'off' ) === 'on' &&
        ...
        if($is_current_ancestor){
            $caret_class = 'fa-'.$posterity_a13->get_option( $depth > 0 ? 'submenu_third_lvl_closer' : 'submenu_closer' );
        }
        else{
