...
$posterity_customizer_dependencies = array();
...
function posterity_customizer_controls($option, &$args, &$wp_customize){
	switch( $option['type'] ) {
		...
		case 'button-set':
			...
			$return = new Posterity_A13_Customize_Button_Set_Control( $wp_customize, $args['setting'], $args );
		...
		case 'wp_dropdown_cpt_terms':
			...
			$args['type']       = 'select';
			$args['choices']    = posterity_cpt_categories_list($option['for']);
		...
		case 'slider':
			...
			$return = new Posterity_A13_Customize_Slider_Control( $wp_customize, $args['setting'], $args);
			break;

		case 'color':
			$args['default']    = isset($option['default']) ? $option['default'] : '';

			$return = new Posterity_A13_Customize_Alpha_Color_Control( $wp_customize, $args['setting'], $args);
			break;

		case 'image':
			$args['default']    = isset($option['default']) ? $option['default'] : '';

			$return = new Posterity_A13_Customize_Image_Control( $wp_customize, $args['setting'], $args);
			break;

		case 'font':
			$args['default']    = isset($option['default']) ? $option['default'] : '';

			$return = new Posterity_A13_Customize_Font_Control( $wp_customize, $args['setting'], $args);
			break;
		...
		case 'spacing':
			...
			$return = new Posterity_A13_Customize_Spacing_Control( $wp_customize, $args['setting'], $args);
			break;
		...
		case 'socials':
			...
			$return = new Posterity_A13_Customize_Socials_Control( $wp_customize, $args['setting'], $args);
			break;

		case 'custom_sidebars':
			$args['default'] = isset($option['default']) ? $option['default'] : '';

			$return = new Posterity_A13_Customize_Sidebars_Control( $wp_customize, $args['setting'], $args);
			break;
...
function posterity_customizer_settings( $wp_customize ) {
	...
	get_template_part('advance/inc/customizer/controls/posterity-class-a13-customize-image-control');
	get_template_part('advance/inc/customizer/controls/posterity-class-a13-customize-alpha-color-control'); 
 	get_template_part('advance/inc/customizer/controls/posterity-class-a13-customize-button-set-control');
	get_template_part('advance/inc/customizer/controls/posterity-class-a13-customize-slider-control');
	get_template_part('advance/inc/customizer/controls/posterity-class-a13-customize-font-control');
	get_template_part('advance/inc/customizer/controls/posterity-class-a13-customize-spacing-control');	
	get_template_part('advance/inc/customizer/controls/posterity-class-a13-customize-socials-control');
	get_template_part('advance/inc/customizer/controls/posterity-class-a13-customize-sidebars-control');
 
	// Register the class so that JS template of controls is available in the Customizer.
	$wp_customize->register_control_type( 'Posterity_A13_Customize_Image_Control' );
	$wp_customize->register_control_type( 'Posterity_A13_Customize_Button_Set_Control' );
	$wp_customize->register_control_type( 'Posterity_A13_Customize_Slider_Control' );
	$wp_customize->register_control_type( 'Posterity_A13_Customize_Font_Control' );
	$wp_customize->register_control_type( 'Posterity_A13_Customize_Spacing_Control' );
	$wp_customize->register_control_type( 'Posterity_A13_Customize_Socials_Control' );
	$wp_customize->register_control_type( 'Posterity_A13_Customize_Sidebars_Control' );

	//sanitization functions
	get_template_part('advance/inc/customizer/sanitization');

	$customizer_structure = $posterity_a13->posterity_get_sections();
	foreach($customizer_structure as $panel){
		$section_priority = 0;

		if( isset($panel['companion_required']) && $panel['companion_required'] === true){
			if( !$posterity_a13->posterity_is_companion_plugin_ready(false, true) ){
		...
		foreach( $sections as $section) {
			...
			foreach( $section['fields'] as $field) {
				...
				//register setting
				$setting_name = POSTERITY_OPTIONS_NAME.'['.$field['id'].']';
				$wp_customize->add_setting( $setting_name, array(
					'default'           => $posterity_a13->theme_options_defaults[ $field['id'] ],
					'type'              => 'option',
					'sanitize_callback' => isset($field['sanitize']) ? 'posterity_sanitize_'.$field['sanitize'] : '',
				...
				if ( isset( $field['required'] ) ) {
					...
					$control_args['active_callback'] = 'posterity_customizer_activate_callback';
					$posterity_customizer_dependencies[ $field['id'] ] = $field['required'];
				}

				$control = posterity_customizer_controls( $field, $control_args, $wp_customize );
				...
				if(isset($field['partial']) && is_array($field['partial'])){
					if(isset($field['partial']['settings'])){
						...
						foreach($field['partial']['settings'] as &$_setting){
							$_setting = POSTERITY_OPTIONS_NAME.'['.$_setting.']';
						}
						unset($_setting);
					}
...
function posterity_customizer_compare_dependency($requirement){
    ...
    $value    = $requirement[2];
	$field_value = $posterity_a13->get_option($id,'',false);
...
function posterity_customizer_activate_callback($control) {
	...
	//lets get field ID
	$matches = array();
	preg_match('/'.POSTERITY_OPTIONS_NAME.'\[([a-z0-9_]+)\]/', $control->id, $matches);

	if(strlen($matches[0])){
		//get requirements from global table
		$requirement = $posterity_customizer_dependencies[ $matches[1] ];

		//control have many requirements
		if ( is_array( $requirement[0] ) ) {
			for ( $i = 0; $i < sizeof( $requirement ); $i ++ ) {
				if ( ! posterity_customizer_compare_dependency( $requirement[ $i ] ) ) {
		...
		//single requirement
			...
			return posterity_customizer_compare_dependency( $requirement );
		}
	}
...
function posterity_customizer_controls_js(){
	...
	wp_enqueue_script('posterity-a13-customize-controls', get_theme_file_uri( 'js/customize-controls.js' ),
		array( 'jquery', 'posterity-admin' ),
	...
	$js_dependencies = array(
		'switches' => array(),
		'dependencies' => $posterity_customizer_dependencies
	);
	foreach($posterity_customizer_dependencies as $field => $dependency){
		//control have many requirements
	...
	wp_add_inline_script( 'posterity-a13-customize-controls', 'A13_CUSTOMIZER_DEPENDENCIES = '.wp_json_encode($js_dependencies));

	$google_fonts_file = get_template_directory().'/advance/inc/google-fonts-json.php';

	$human_variants = array(
		'100' => esc_html__( 'thin', 'posterity' ),
		'200' => esc_html__( 'extra-light', 'posterity' ),
		'300' => esc_html__( 'light', 'posterity' ),
		'400' => esc_html__( 'regular', 'posterity' ),
		'500' => esc_html__( 'medium', 'posterity' ),
		'600' => esc_html__( 'semi-bold', 'posterity' ),
		'700' => esc_html__( 'bold', 'posterity' ),
		'800' => esc_html__( 'extra-bold', 'posterity' ),
		'900' => esc_html__( 'black', 'posterity' ),
	);

	$skt_params = array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'options_name' => POSTERITY_OPTIONS_NAME,
		'standard_fonts' => wp_json_encode($posterity_a13->posterity_get_standard_fonts_list()),
...
function posterity_customizer_preview_js(){
	...
	wp_localize_script( 'posterity-a13-customize-preview', 'A13FECustomizerPreview', $skt_params );
...
function posterity_customizer_preview_css(){
	global $wp_styles;
	//CSS
	posterity_enable_user_css_functions();
	$css = posterity_get_user_css(false);
	//all the settings from theme options that generates CSS
	echo '<style type="text/css" media="all" id="user-css-inlined">'.esc_html($css).'</style>';
	//only custom CSS - so we can update it live
	echo '<style type="text/css" media="all" id="user-custom-css">'.esc_html(posterity_user_custom_css()).'</style>';
...
function posterity_prepare_partial_css($response, $option, $function) {
	...
	if(isset($response['contents'][$partial_name])){
		$css_option_id = POSTERITY_OPTIONS_NAME.'-'.$option;
