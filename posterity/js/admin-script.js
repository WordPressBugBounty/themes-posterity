...
(function($){
    ...
    window.POSTERITY_ADMIN = {
        ...
        onReady : function(){
            ...
            POSTERITY_ADMIN.upload();
            POSTERITY_ADMIN.utils.init();
            POSTERITY_ADMIN.metaActions.init();
            POSTERITY_ADMIN.settingsAction();
			POSTERITY_ADMIN.demoDataImporter();
			POSTERITY_ADMIN.registerLicense();
			POSTERITY_ADMIN.installCompanionPlugin();
		},

        installCompanionPlugin : function(){

            var plugin_card = $('.plugin-card-skt-templates');
                ...
                var log_div                = $('#demo_data_import_log').children('div'),
					...
					nextLevel = function(level, sublevel){
						...
						$.ajax({
							...
							url: ajaxurl,
							data:  {
								action : 'posterity_import_demo_data', //called in backend
								level : level,
								sublevel : sublevel,
                                demo_id : demo_id,
                        ...
                        item.one('itemAnimated', function(){
                            if(item.hasClass('open')){
                                ...
                                if(full.length){
                                    ...
                                    full = $('<img class="full" />').attr('src', item.data('full')).hide().load(function(){
                ...
                var submit              = license_input.next('span.code_submit'),
                    check_purchase_code = function (purchase_code) {
                        ...
                        $.ajax({
                            ...
                            dataType: "json",
                            data    : {
                                action: 'posterity_check_license_code', //called in backend
                                code  : purchase_code,
...
//                                    current_input.focus();
                ...
                if(import_button.length){
                    var import_settings = function (settings) {
                        ...
                        $.ajax({
                            ...
                            dataType: "json",
                            data    : {
                                action: 'posterity_import_theme_settings', //called in backend
                                settings  : settings,
        ...
        metaActions : {
            init : function(){
                ...
                var skt_meta = $('div.sktwb-metas'),
                    AM = POSTERITY_ADMIN.metaActions;

                if (skt_meta.length) {
                    //bind multi upload and some other things
                    AM.muManage(skt_meta);

                    //bind switcher(hides unused options like image vs video)
                    skt_meta.on('change','input[type="radio"], select',{}, AM.checkDependencies);
			...
			muManage : function(skt_meta){
                var prototype_selector = 'div.fieldset.prototype',
					_prototype = skt_meta.find(prototype_selector);
                ...
                if(_prototype.length){
                    ...
                    var items_JSON              = JSON.parse( textarea.val() ),
						our_skt_meta			= textarea.parents('.sktwb-metas').eq(0),
                            ...
                            if (items_num) {
								...
								if(to_send_array.length){
									$.ajax({
										...
										url: ajaxurl,
										data: {
											action : 'posterity_prepare_gallery_items_html', //called in backend
											items : to_send_array
										},
                        ...
                        updateItem = function(){
							...
							if(item.id === 'external'){
								...
								$.ajax({
									...
									url: ajaxurl,
									data: {
										action : 'posterity_prepare_gallery_single_item_html', //called in backend
										item : item
									},
									success: function(new_html) {
										//insert HTML
                        ...
                        if(dependency.length){
                            ...
                            if(Array.isArray(dependency[0])){
                                for(var i = 0; i < dependency.length; i++ ){
                                    if(!POSTERITY_ADMIN.metaActions.compareDependency(dependency[i], possible_switches)){
                            ...
                            //we have only one required condition
                                ...
                                if(!POSTERITY_ADMIN.metaActions.compareDependency(dependency, possible_switches)){
        ...
        settingsAction : function(){
            ...
            POSTERITY_ADMIN.ratingNotice();
            POSTERITY_ADMIN.proofingNotice();
            POSTERITY_ADMIN.adminNotices();
			...
			$('#vc_ui-panel-edit-element').on('click','.a13_delete_selected_nava', function(){
                ...
                if(confirm(G.messages.confirm_delete_nava)){
                    $.ajax({
                        type: 'post',
                        url: SKTParams.ajaxurl,
                        data: {
                            action: 'posterity_nava_delete_post',
                            id: id
                        },
            ...
            .on('keyup','.a13_new_nava_id', function(e){
                ...
                $.ajax({
                    ...
                    url: SKTParams.ajaxurl,
                    data: {
                        action: 'posterity_nava_add_post',
                        title: name
                    },
            ...
            if(rating_notice.length){
                ...
                links.on('click', function(e){
                    ...
                    $.ajax({
                        ...
                        url: ajaxurl,
                        data: {
                            action : 'posterity_rating_notice_action', //called in backend
            ...
            if(proofing_notice.length){
                $('#hide-proof-notice').on('click', function(e){
                    ...
                    $.ajax({
                        ...
                        url: ajaxurl,
                        data: {
                            action : 'posterity_proofing_notice_action' //called in backend
                        },
                        success: function(reply) {
                            //console.log(reply);
                        },
            ...
            if(notices.length){
                notices.on('click', '.notice-dismiss', function(e){
                    ...
                    $.ajax({
                        ...
                        url: G.ajaxurl,
                        data: {
                            action : 'posterity_disable_ajax_notice', //called in backend
							nonce: SKTParams.ajax_nonce,
    ...
    var POSTERITY_ADMIN = window.POSTERITY_ADMIN;

    //start ADMIN
    $(document).ready(POSTERITY_ADMIN.onReady);
